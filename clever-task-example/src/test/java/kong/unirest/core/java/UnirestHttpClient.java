...
/**
 * {@link kong.unirest.core.java.JavaClient}
 * <p>
 * 作者：lizw <br/>
 * 创建时间：2026/01/20 10:46 <br/>
 */
public class UnirestHttpClient implements Client {
    private final Config config;
    private final HttpClient client;

    public UnirestHttpClient(Config config) {
        this.config = config;
        this.client = new JavaClientBuilder().apply(config);
    }

    public UnirestHttpClient(Config config, HttpClient client) {
        this.config = config;
        this.client = client;
    }
    ...
    private java.net.http.HttpRequest getRequest(HttpRequest<?> request) {
        try {
            ...
            var jreq = newBuilder(url)
                ...
                .method(request.getHttpMethod().name(), new BodyBuilder(request).getBody());
    ...
    private void setHeaders(HttpRequest<?> request, java.net.http.HttpRequest.Builder jreq) {
        request.getHeaders().all().forEach(h -> {
            // Expect", "100-continue"
            if ("Expect".equalsIgnoreCase(h.getName()) && "100-continue".equalsIgnoreCase(h.getValue())) {
                jreq.expectContinue(true);
                return;
            }
            jreq.header(h.getName(), h.getValue());
        });
    ...
    public <T> CompletableFuture<HttpResponse<T>> request(HttpRequest request, Function<RawResponse, HttpResponse<T>> transformer, CompletableFuture<HttpResponse<T>> callback, Class<?> resultType) {
        ...
        var requestObj = getRequest(request);
        var metric = config.getMetric().begin(reqSum);
        var execute = client.sendAsync(requestObj, java.net.http.HttpResponse.BodyHandlers.ofInputStream());
